...
namespace IslandBoy
{
	public class TCCraftingUI : MonoBehaviour
	{
		...
		private void UpdateIngTexts()
		{
			...
			{
				string text = $"{ ia.Item.Name} [{_po.Inventory.GetItemAmount(ia.Item)}/{ia.Amount * _recipeAmount}]";

				if(_po.Inventory.GetItemAmount(ia.Item) >= (ia.Amount * _recipeAmount))
		...
		private bool CheckIfCanCraft()
		{
			...
			foreach (ItemAmount ia in _recipeToDisplay.ResourceList)
			{
				canCraft = _po.Inventory.Contains(ia.Item, ia.Amount * _recipeAmount);
		...
		private bool CanIncrease()
		{
			foreach (var ia in _recipeToDisplay.ResourceList)
			{
				int amountInInv = _po.Inventory.GetItemAmount(ia.Item);
